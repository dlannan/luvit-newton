...
function simApp:makePin( x, y, z, r, h)

    idc = idc + 1
    local coll = gnewt.NewtonCreateCylinder( self.client, r, r, h, idc, nil )
    -- create a dynamic body with a sphere shape, and     
    --local iM = gutil.identityMatrix()
    local xaxis = ffi.new("dVector", { 0.0, 0.0, 1.0 })
    local iM = vis:rotationMatrix( xaxis, math.pi * 0.5 )
    iM[0].m_posit.m_x = x
    iM[0].m_posit.m_y = y
    iM[0].m_posit.m_z = z
    local body = self:addBody( iM, PIN_MASS, coll )
    -- do no forget to destroy the collision after you not longer need it
...
function simApp:makePins( x, y, z, depth )
    ...
    for h=1, depth, 1 do
        local wcount = h
        local xstart = x - wcount * 0.5 + 0.5

        for l=1, wcount, 1 do

            local x = xstart + (l-1)
            local z = zstart + (h-1)
            bodies[bid] = self:makePin( x, y, z, PIN_RADIUS, PIN_HEIGHT )
            bid = bid + 1   
        end
    end
...
function simApp:renderPins( wall )
    ...
    for k,v in pairs(wall) do
        ...
        vis:DrawCylinder( 6, 6, PIN_HEIGHT, PIN_RADIUS )
        gl.glPopMatrix()
    end
end
...
function simApp:Startup()
    ...
    if VISUAL then
        ...
        self.window = glfw.CreateWindow(640, 480, "Sample02 - bowling")
        p(self.window)
    ...
    ------------------------------------------------------------------------------------------------------------
    -- Make the wall
    self.wall = self:makePins( 0, 0.1, 5, 4 )
...
function simApp:Render()
    ...
    if self.wall then self:renderPins(self.wall) end
